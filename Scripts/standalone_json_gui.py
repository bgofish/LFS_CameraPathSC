...
Compatible with LichtfeldStudio camera format.
"""
...
class StandaloneJSONGUI:
    def __init__(self, root):
        ...
        # Animation variables
        self.animation_type = tk.StringVar(value="spiral")
        ...
        self.end_radius = tk.DoubleVar(value=5.0)
        self.start_height = tk.DoubleVar(value=0.0)
        self.end_height = tk.DoubleVar(value=10.0)
        
        # Spiral follow-Y parameters
        self.spiral_follow_y = tk.BooleanVar(value=True)
        self.spiral_y_offset = tk.DoubleVar(value=0.0)

        # Orientation offset variables (steps of 90 degrees each)
        self.orient_offset_pitch = tk.IntVar(value=2)
        self.orient_offset_yaw   = tk.IntVar(value=0)
        self.orient_offset_roll  = tk.IntVar(value=2)
        ...
        self.keyframe_step = tk.IntVar(value=10)  # Keyframe every N frames
    ...
    def create_widgets(self):
        ...
        # ttk.Checkbutton(json_frame, text="Convert coordinates to LichtfeldStudio Y-up system", 
        ...
        # Orientation offset section
        orient_frame = ttk.LabelFrame(main_frame, text="Orientation Offsets (90° steps)  —  use to dial in correct camera orientation", padding="10")
        orient_frame.grid(row=row, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=(10, 5))

        def _deg_label(var):
            """Return a StringVar that shows the current offset in degrees."""
            sv = tk.StringVar()
            def update(*_):
                sv.set(f"{var.get() * 90:+d}°")
            var.trace_add("write", update)
            update()
            return sv

        for col, (label, var) in enumerate([("Pitch (X)", self.orient_offset_pitch),
                                             ("Yaw (Y)",   self.orient_offset_yaw),
                                             ("Roll (Z)",  self.orient_offset_roll)]):
            ttk.Label(orient_frame, text=label + ":").grid(row=0, column=col*3, sticky=tk.W, padx=(0 if col==0 else 20, 5))
            spin = tk.Spinbox(orient_frame, textvariable=var,
                              from_=-3, to=3, increment=1, width=4,
                              command=lambda v=var: v.set(max(-3, min(3, v.get()))))
            spin.grid(row=0, column=col*3+1, sticky=tk.W, padx=(0, 5))
            ttk.Label(orient_frame, textvariable=_deg_label(var), width=5,
                      foreground="blue").grid(row=0, column=col*3+2, sticky=tk.W)

        ttk.Button(orient_frame, text="+Y Up LFS=NB",
                   command=lambda: [self.orient_offset_pitch.set(2),
                                    self.orient_offset_yaw.set(0),
                                    self.orient_offset_roll.set(2)]
                   ).grid(row=0, column=10, padx=(8, 0))

        ttk.Button(orient_frame, text="-Y Up LFS<=0.5.1",
                   command=lambda: [self.orient_offset_pitch.set(2),
                                    self.orient_offset_yaw.set(2),
                                    self.orient_offset_roll.set(2)]
    ...
    def generate_json_thread(self):
        ...
        try:
            ...
            # When spiral follow-Y is active, target must be None so the generator
            # enters the auto-target branch where follow-Y is applied.
            force_no_target = (self.animation_type.get() == "spiral" and self.spiral_follow_y.get())
            target = None if (self.use_auto_target.get() or force_no_target) else (self.target_x.get(), self.target_y.get(), self.target_z.get())
            ...
            data = generator.generate_camera_animation(
                ...
                orient_offset_pitch=self.orient_offset_pitch.get(),
                orient_offset_yaw=self.orient_offset_yaw.get(),
                orient_offset_roll=self.orient_offset_roll.get(),
            )
            ...
            self.log_message("")
            self.log_message("Ready for LichtfeldStudio:")
            self.log_message("1. Open LichtfeldStudio")
            self.log_message("2. Open Sequencer")
            self.log_message("3. Load your camera animation json in the Sequencer panel")
            
            # Show success message
            # self.root.after(0, lambda: messagebox.showinfo(
                # "Success", 
                # f"JSON file generated successfully!\n\nLocation: {self.output_path.get()}\n\nYou can now read this into LichtfeldStudio."
            # ))
    ...
    def generate_json(self):
        ...
        p = self.orient_offset_pitch.get() * 90
        y = self.orient_offset_yaw.get()   * 90
        r = self.orient_offset_roll.get()  * 90
        if p or y or r:
            self.log_message(f"Orientation Offset: P={p:+d}°  Y={y:+d}°  R={r:+d}°")
        else:
            self.log_message("Orientation Offset: none")
